...
data<-read.table(file="hearth_database.csv",header=T,sep=",")
head(data)
```
...
```
A continuacion se calcula  Matriz de correlación de Pearson:
```{r}
# oldpeak:
cor(data[7:14])
```
...
```
Dada la diferencia de maginitudes entre los campos, se obtendran los boxplot de manera individual:
```{r}
# boxplot:
...
```
A continuacion se visualiza a través de un histograma como distribuyen las variables respecto a los TARGET:
...
cat("Probabilidad de salir cara: \n")
B[1000]
```
...
n_juegos <- function(n = 10000 ,cambiar_puerta = TRUE){
  ...
  A<-c(A,sum(z)/i) # Se suman los aciertos de la ultima iteración, lo cual permite contar el número de exitos, dividido por el número de exitos
  }
  return(A)}
A<-n_juegos()
# Gráfico de la convergencia
...
cat("Probabilidad de ganar al cambiar de puerta: \n")
A[10000]
```
